...
struct LSTMCell : Cell<std::tuple<Tensor, Tensor>, cell_params> {
  ...
  hidden_type operator()(const Tensor& input, const hidden_type& hidden, const cell_params& params) const override {
    ...
    //rit_begin
    auto master_gates = gates.narrow(1,0,2).chunk(2,1);
    auto cingate = master_gates[0].softmax(-1).cumsum(-1);
    auto cforgetgate = master_gates[1].softmax(-1).cumsum(-1);
    auto distance_cforget = cforgetgate.sum(-1);
    auto distance_cin = cingate.sum(-1);
    //rit_end
    ...
    //rit_begin
    auto overlap = (cforgetgate * cingate);
